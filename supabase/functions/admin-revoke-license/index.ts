  ...
  try {
    ...
    if (reseller && reseller.status === "active") {
      ...
      if (!license || license.reseller_id !== reseller.id) {
        throw new Error("Você só pode revogar licenças criadas por você");
      }
    } else {
      // É admin: validar permissão
    ...
    await adminClient.from("admin_audit_logs").insert({
      ...
      license_key,
      metadata: { revoked_by: reseller ? "reseller" : "admin" }
    });

    return new Response(
      JSON.stringify({ success: true, data, message: "Licença revogada com sucesso" }),
